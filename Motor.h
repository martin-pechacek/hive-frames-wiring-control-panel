...
enum MotorMode { AUTOMATIC, MANUAL };

// library interface description
class Motor {
  public:
    // constructors:
    Motor(int number_of_steps, int motor_pin_1, int motor_pin_2, Button* pauseBtn);
    ~Motor(){};

    void setMode(MotorMode mode);
    void setPauseBtn(Button btn);
  ...
  private:
    bool stepMotor(int this_step);
    bool pause();

    int direction;            // Direction of rotation
    unsigned long step_delay; // delay between steps, in ms, based on speed
    int number_of_steps;      // total number of steps this motor can take
    const int pin_count = 2;  // how many pins are in use.
    ...
    const unsigned int AUTOMATIC_CYCLE_STEPS = 23600;
    const unsigned int MANUAL_CYCLE_STEPS = 1000;
    const unsigned long SPEED = 360;
};
